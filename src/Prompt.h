      ...
      for (unsigned i = 0; i < revisedUserInput.size(); ++i){ //search through parsed input for connectors.
        ...
        //}
          ...
          if (subTree.empty()) { //if tree is empty, push a command
            ...
            subTree.back()->setRight(new File(commandInput)); 
          }
          ...
          subTree.push_back(new Pipe());
          subTree.back()->setLeft(tempLeft); //sets AND left to tempLeft
          //TODO add the else and the obj creation
        }
        else if(revisedUserInput.at(i) == ">"){
          isRedirect = 1;
          if (subTree.empty() && tempLeft == NULL) {
          ...
          subTree.push_back(new Overwrite());
          subTree.back()->setLeft(tempLeft); //sets AND left to tempLeft
          ...
          subTree.push_back(new Append());
          subTree.back()->setLeft(tempLeft); //sets AND left to tempLeft
          
        }
        else if(revisedUserInput.at(i) == "<"){ //TODO deal with this later
          isRedirect = 1;
          if (subTree.empty() && tempLeft == NULL) {
            tempLeft = new Command(commandInput); //sets tempLeft child to <
            commandInput.clear();     //clears commandInput for other commands
          }
          else {  //if connector has no left and the list is not empty
            tempLeft = subTree.back();
        ...
        else if (tempLeft != NULL && ( (revisedUserInput.at(i + 1) == ">") || (revisedUserInput.at(i + 1) == ">>") || (revisedUserInput.at(i + 1) == "<") ) ) { //**TESTING added ")""
            commandInput.push_back(revisedUserInput.at(i));
            tempRight = new File(commandInput);
            commandInput.clear();
            subTree.back()->setRight(tempRight);
            tempLeft = NULL;
            tempRight = NULL;
          }
        else if (tempLeft != NULL && ( (revisedUserInput.at(i + 1) == "&&") || (revisedUserInput.at(i + 1) == "||") || (revisedUserInput.at(i + 1) == ";") || (revisedUserInput.at(i + 1) == ")") || (revisedUserInput.at(i + 1) == "|") ) ) { //**TESTING added ")""
            commandInput.push_back(revisedUserInput.at(i));
